...
import { useIdentityWheel } from '@/hooks/useIdentityWheel';
import { identityWheelOptions } from '@/data/identityWheelOptions';
...
const IdentityWheel = ({ onComplete, resetKey = 0 }: IdentityWheelProps) => {
  const {
    spinning,
    selectedOption,
    response,
  ...
  const getSegmentColors = () => {
    const segmentCount = identityWheelOptions.length;
    const segmentSize = 360 / segmentCount;
    
    return identityWheelOptions.map((option, index) => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto px-4 py-6">
      <h2 className="text-2xl font-bold mb-6">Identity Wheel</h2>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    {Object.entries(completedOptions).map(([id, responses]) => {
                      const option = identityWheelOptions.find(opt => opt.id === id);
                      return (

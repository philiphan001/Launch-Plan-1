...
export class CareerSearchService {
  ...
  async search(filters: CareerSearchFilters): Promise<Career[]> {
    ...
    try {
      ...
      if (filters.growthRate)
        queryParams.append("growthRate", filters.growthRate.toString());
      if (filters.jobType) queryParams.append("jobType", filters.jobType);
      if (filters.fieldOfStudy)
        queryParams.append("fieldOfStudy", filters.fieldOfStudy);
      if (filters.globalSearch)
        queryParams.append("globalSearch", filters.globalSearch.toString());

      const response = await this.retryOperation(async () => {
        const res = await fetch(
          `/api/careers/search?${queryParams.toString()}`
        );
        if (!res.ok) {
  ...
  async addMultipleToFavorites(
    ...
    try {
      ...
      for (const careerId of careerIds) {
        const isAlreadyFavorited = await this.isCareerFavorited(
          userId,
          careerId
        );
  ...
  async getSuggestedCareersByField(fieldOfStudy: string): Promise<Career[]> {
    try {
      const response = await this.retryOperation(async () => {
        const res = await fetch(
          `/api/careers/suggestions/${encodeURIComponent(fieldOfStudy)}`
        );
        if (!res.ok) {
  ...
  async getTopCareersByField(
    ...
    try {
      const response = await this.retryOperation(async () => {
        const res = await fetch(
          `/api/careers/top/${encodeURIComponent(fieldOfStudy)}?limit=${limit}`
        );
        if (!res.ok) {
    ...
    try {
      ...
      if (includeSuggestedCareers) {
        const suggestedCareers =
          await this.getSuggestedCareersByField(fieldOfStudy);
        // Filter out duplicates
        const uniqueSuggestedCareers = suggestedCareers.filter(
          (suggested) =>
            !results.some((existing) => existing.id === suggested.id)
        );

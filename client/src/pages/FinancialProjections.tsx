...
import {
  createMainProjectionChart,
  fixLiabilityCalculation,
} from "@/lib/charts";
import {
...
import ProjectionSummary, {
  ProjectionSummaryData,
} from "@/components/financial/ProjectionSummary";
...
const FinancialProjections = ({
  ...
  // Use the authenticated user's ID, or return null if not available
  const userId: number | null = user?.id ?? null;

  // Guard to ensure userId is a number before proceeding
  if (userId === null) {
    return <div>Please log in to view your financial projections.</div>;
  }
  ...
  const { projectionId, timestamp, autoGenerate, fromJobPathway } =
    useMemo(() => {
  ...
  const [taxTableSectionOpen, setTaxTableSectionOpen] =
    useState<boolean>(false); // Tax table starts collapsed
  const [advancedSettingsOpen, setAdvancedSettingsOpen] =
    useState<boolean>(false); // Advanced settings start collapsed
  const [calculationsSectionOpen, setCalculationsSectionOpen] =
  ...
  const [isLoadingProjection, setIsLoadingProjection] =
  ...
  const { data: favoriteColleges, isLoading: isLoadingFavoriteColleges } =
    useQuery({
      queryKey: ["/api/favorites/colleges", userId],
      queryFn: async () => {
        // Only call if userId is not null
        if (!userId) return [];
        return FavoritesService.getFavoriteColleges(userId);
      },
      enabled: !!userId && autoGenerate,
    });

  // Fetch favorite careers
  const { data: favoriteCareers, isLoading: isLoadingFavoriteCareers } =
    useQuery({
      queryKey: ["/api/favorites/careers", userId],
      queryFn: async () => {
        // Only call if userId is not null
        if (!userId) return [];
        return FavoritesService.getFavoriteCareers(userId);
      },
      enabled: !!userId && autoGenerate,
    });
    ...
    useQuery({
      ...
      queryFn: async () => {
        try {
          // Only call if userId is not null
          if (!userId) return [];
          return FavoritesService.getFavoriteLocations(userId);
        } catch (error) {
  ...
  const {
    ...
    isLoading: isLoadingFinancialProfile,
    error: financialProfileError,
  } = useQuery({
  ...
  const { data: locationCostData, isLoading: isLoadingLocationData } = useQuery(
    {
      ...
      queryFn: async () => {
        ...
        if (!response.ok)
          throw new Error("Failed to fetch location cost of living data");
        ...
        console.log("Location cost of living data:", {
          ...
          healthcare_exists:
            data?.healthcare !== undefined && data?.healthcare !== null,
  ...
  const { data: collegeCalculations, isLoading: isLoadingCollegeCalcs } =
    useQuery({
  ...
  const { data: careerCalculations, isLoading: isLoadingCareerCalcs } =
    useQuery({
  ...
  const { data: savedProjections = [], isLoading: isLoadingSavedProjections } =
    useQuery({
  ...
  useEffect(() => {
    if (careers && careers.length > 0) {
      ...
      console.log(
        "Made careers data globally available for Python calculator:",
        careers.length
      );
    }
  }, [careers]);
  ...
  const deleteMilestonesMutation = useMutation({
    mutationFn: async (userId: number) => {
      ...
      if (!response.ok) {
        console.error(
          "Failed to delete existing milestones:",
          response.statusText
        );
        return false;
      }
  ...
  const addCollegeToFavoritesMutation = useMutation({
    mutationFn: (collegeId: number) =>
      FavoritesService.addCollegeToFavorites(userId as number, collegeId),
  ...
  const addCareerToFavoritesMutation = useMutation({
    mutationFn: (careerId: number) =>
      FavoritesService.addCareerToFavorites(userId as number, careerId),
  ...
  useEffect(() => {
    if (autoGenerate && startGeneration && !deletionComplete) {
      if (!isAuthenticated || !deleteMilestonesMutation.isPending) {
        console.log(
          "Milestone deletion complete or not needed, proceeding with financial projection generation"
        );
    ...
    autoGenerate,
    startGeneration,
    deleteMilestonesMutation.isPending,
    deletionComplete,
    isAuthenticated,
  ]);
  ...
  useEffect(() => {
    if (
      autoGenerate &&
      (deletionComplete || !isAuthenticated) &&
      !pathwayDataProcessed
    ) {
      console.log(
        "Auto-generating financial projection from pathway and favorite data"
      );

      try {
        console.log(
          "Checking for pathway data from localStorage and favorite data..."
        );
        ...
        if (pathwayDataStr) {
          ...
          try {
            ...
            if (isAuthenticated && userId) {
              ...
              if (
                ...
                if (pathwayData.selectedSchoolId) {
                  ...
                  const lastAddedCollegeId =
                    localStorage.getItem("lastAddedCollegeId");
                  if (
                    lastAddedCollegeId !== String(pathwayData.selectedSchoolId)
                  ) {
                    console.log(
                      "Adding college to favorites from pathway data:",
                      pathwayData.selectedSchoolId
                    );
                    addCollegeToFavoritesMutation.mutate(
                      pathwayData.selectedSchoolId
                    );
                    // Store that we've added this college
                    localStorage.setItem(
                      "lastAddedCollegeId",
                      String(pathwayData.selectedSchoolId)
                    );
                  } else {
                    console.log(
                      "College was already added to favorites in this session:",
                      pathwayData.selectedSchoolId
                    );
                  }
                }
              ...
              if (pathwayData.selectedCareer) {
                ...
                const lastAddedCareerId =
                  localStorage.getItem("lastAddedCareerId");
                if (lastAddedCareerId !== String(pathwayData.selectedCareer)) {
                  console.log(
                    "Adding career to favorites from pathway data:",
                    pathwayData.selectedCareer
                  );
                  addCareerToFavoritesMutation.mutate(
                    pathwayData.selectedCareer
                  );
                  // Store that we've added this career
                  localStorage.setItem(
                    "lastAddedCareerId",
                    String(pathwayData.selectedCareer)
                  );
                } else {
                  console.log(
                    "Career was already added to favorites in this session:",
                    pathwayData.selectedCareer
                  );
                }
              }
            ...
            console.error(
              "Error parsing pathway data from localStorage:",
              error
            );
            ...
            toast({
              ...
              description:
                "There was an issue generating your financial plan. Default values will be used.",
              variant: "destructive",
            });
          }
        } else {
          console.log(
            "No pathway data in localStorage, using favorite data instead"
          );
        }
        ...
        if (favoriteColleges && favoriteColleges.length > 0) {
          ...
          if (collegeCalculations && collegeCalculations.length > 0) {
            ...
            const favoriteCollegeIds = favoriteColleges.map(
              (fc: any) => fc.collegeId
            );
            ...
            if (matchingCollegeCalc) {
              console.log(
                "Found matching college calculation for favorite:",
                matchingCollegeCalc
              );
              ...
              if (!matchingCollegeCalc.includedInProjection) {
                ...
                console.log(
                  "Setting this college calculation as included in projection"
                );
              }
            }
          }
        ...
        if (favoriteCareers && favoriteCareers.length > 0) {
          ...
          if (careerCalculations && careerCalculations.length > 0) {
            const favoriteCareerIds = favoriteCareers.map(
              (fc: any) => fc.careerId
            );
            ...
            if (matchingCareerCalc) {
              console.log(
                "Found matching career calculation for favorite:",
                matchingCareerCalc
              );
              ...
              if (!matchingCareerCalc.includedInProjection) {
                // This would ideally update the database, but we'll just use the value for now
                console.log(
                  "Setting this career calculation as included in projection"
                );
              }
            }
          }
          ...
          if (startingIncome === 40000 && careers && careers.length > 0) {
            ...
            const careerInfo = careers.find(
              (c: any) => c.id === favoriteCareer.careerId
            );

            if (careerInfo) {
              console.log("Found career info for favorite:", careerInfo);
              startingIncome =
                careerInfo.salary_pct_10 || // Use 10th percentile as entry level
                careerInfo.salary ||
                40000;
            }
          }
        } else if (
          pathwayData?.selectedProfession &&
          careers &&
          careers.length > 0
        ) {
          ...
          const selectedCareer = careers.find(
            (c: any) =>
              c.title.toLowerCase() ===
              pathwayData.selectedProfession.toLowerCase()
          );

          if (selectedCareer) {
            startingIncome =
              selectedCareer.salary_pct_10 || selectedCareer.salary || 40000;
          }
        }
        ...
        if (favoriteLocations && favoriteLocations.length > 0) {
          ...
          if (typeof pathwayData.location === "string") {
            ...
            console.log(
              "Using string location from pathway data:",
              pathwayData.location
            );
            ...
            console.log(
              "Using object location from pathway data:",
              pathwayData.location.zipCode
            );
            locationZipCode = pathwayData.location.zipCode;
          } else if (pathwayData.zipCode) {
            // Direct zipCode property
            console.log(
              "Using zipCode from pathway data:",
              pathwayData.zipCode
            );
            locationZipCode = pathwayData.zipCode;
          } else {
            console.log(
              "Pathway data exists but no location found:",
        ...
        if (locationZipCode && locationZipCode !== userData?.zipCode) {
          ...
          if (locationCostData) {
            ...
            setExpenses(
              Math.max(
                Math.round(annualExpenses),
                Math.round(startingIncome * 0.5)
              )
            );
          } else {
      ...
      if (!isInLoadingState) {
        console.log(
          "DEFAULT INITIALIZATION: Not in loading state, applying saved calculations"
        );

        // Set student loan debt from college calculations
        if (collegeCalculations && collegeCalculations.length > 0) {
          // Find the college calculation that's included in projections
          const includedCollegeCalc = collegeCalculations.find(
            (calc) => calc.includedInProjection
          );
          if (includedCollegeCalc) {
            console.log(
              "DEFAULT INITIALIZATION: Using student loan debt from college calculation:",
        ...
        if (careerCalculations && careerCalculations.length > 0) {
          ...
          const includedCareerCalc = careerCalculations.find(
            (calc) => calc.includedInProjection
          );
          if (includedCareerCalc && includedCareerCalc.entryLevelSalary) {
            console.log(
              "DEFAULT INITIALIZATION: Using entry level salary from career calculation:",
              includedCareerCalc.entryLevelSalary
            );
            setIncome(includedCareerCalc.entryLevelSalary);
          } else if (includedCareerCalc) {
            console.log(
              "DEFAULT INITIALIZATION: Using projected salary from career calculation:",
        ...
        if (income > 0) {
          ...
          if (locationCostData) {
            ...
            // as a sanity check (people generally don't spend less than half their income)
            console.log(
              "DEFAULT INITIALIZATION: Setting expenses based on location data:",
              Math.max(Math.round(annualExpenses), Math.round(income * 0.5))
            );
            setExpenses(
              Math.max(Math.round(annualExpenses), Math.round(income * 0.5))
            );
            ...
            console.log(
              "DEFAULT INITIALIZATION: Setting expenses based on income percentage:",
              Math.round(baseExpenses)
            );
        ...
        console.log(
          "DEFAULT INITIALIZATION: In loading state, skipping saved calculations to preserve loaded projection data"
        );
      }
    }
  }, [
  ...
  const [personalLoanInterestRate, setPersonalLoanInterestRate] =
    useState<number>(8.0); // Default: 8.0% annual interest
  ...
  const getAssumptionValue = (
    ...
    const assumption = assumptionsData.find(
      (a: any) => a.category === category && a.key === key && a.isEnabled
    );
  ...
  useEffect(() => {
    if (assumptionsData && assumptionsData.length > 0) {
      console.log(
        "Processing assumptions for financial calculations:",
        assumptionsData
      );
      ...
      console.log(
        "Spouse asset growth assumption:",
        spouseAssetGrowthAssumption
      );

      // Set state variables with assumption values or use defaults
      if (spouseLoanTermAssumption) {
        console.log(
          "Setting spouse loan term to:",
          spouseLoanTermAssumption.value
        );
      ...
      if (spouseLoanRateAssumption) {
        console.log(
          "Setting spouse loan rate to:",
          spouseLoanRateAssumption.value
        );
      ...
      if (spouseAssetGrowthAssumption) {
        console.log(
          "Setting spouse asset growth to:",
          spouseAssetGrowthAssumption.value
        );
  ...
  const isLoading =
    ...
    isLoadingCollegeCalcs ||
    isLoadingCareerCalcs ||
  ...
  const years =
    timeframe === "5 Years" ? 5 : timeframe === "20 Years" ? 20 : 10;
  ...
  const generateProjectionData = (milestonesList = milestones) => {
    ...
    currentExpenses = Math.max(
      currentIncome * 0.5,
      Math.min(currentExpenses, currentIncome * 0.9)
    );
    ...
    const homeMilestone = sortedMilestones.find((m) => m.type === "home");
    const carMilestone = sortedMilestones.find((m) => m.type === "car");
    const marriageMilestone = sortedMilestones.find(
      (m) => m.type === "marriage"
    );
    ...
    for (let i = 1; i <= years; i++) {
      ...
      if (hasSpouse) {
        spouseIncome = Math.round(
          spouseIncome * (1 + spouseIncomeGrowth / 100)
        );
      }
        ...
        yearMilestones.forEach((milestone: any) => {
          ...
          switch (milestone.type) {
            case "marriage":
              ...
              console.log(
                `Marriage milestone: Adding spouse with income $${spouseIncome}, assets $${spouseAssets}, and liabilities $${spouseLiabilities}`
              );
              break;
            ...
            case "home":
              ...
              console.log(
                `Home milestone: Value $${homeValue}, Down payment $${downPayment}, Mortgage $${mortgagePrincipal}, annual payment $${mortgagePayment}`
              );
              break;
            ...
            case "car":
              ...
              console.log(
                `Car milestone: Value $${carValue}, Down payment $${carDownPayment}, Loan $${carLoanPrincipal}, annual payment $${carPayment}`
              );
              console.log(
                `Transportation expenses reduced by ${carTransportationReduction * 100}% due to car purchase`
              );
              break;
            ...
            case "children":
              ...
              childrenExpenses =
                (milestone.childrenExpensePerYear || 0) *
      ...
      if (remainingStudentLoanDebt > 0 && i <= 10) {
        ...
        const amortizedAnnualPayment =
          (studentLoanDebt * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
      ...
      if (remainingStudentLoanDebt > 0 && i <= 10) {
        ...
        const annualInterestPaid =
          remainingStudentLoanDebt * studentLoanInterestRate;
        ...
        educationExpensesData[i - 1] =
          (educationExpensesData[i - 1] || 0) + amortizedAnnualPayment;

        console.log(
          `Student loan payment for year ${i}: $${amortizedAnnualPayment.toFixed(2)} (interest: $${annualInterestPaid.toFixed(2)}, principal: $${principalReduction.toFixed(2)})`
        );
        console.log(
          `Student loan balance: $${remainingStudentLoanDebt.toFixed(2)} → $${(remainingStudentLoanDebt - principalReduction).toFixed(2)}`
        );

        // Update the student loan balance for this year
        remainingStudentLoanDebt = Math.max(
          0,
          remainingStudentLoanDebt - principalReduction
        );
      }
      ...
      if (
        projectionData?.educationLoans &&
        projectionData.educationLoans[i - 1] !== undefined
      ) {
        ...
        if (undergraduateLoans > 0 || graduateLoans > 0) {
          console.log(
            `Education loans for year ${i}: Undergraduate $${undergraduateLoans}, Graduate $${graduateLoans}`
          );
        }
      }
      ...
      if (hasHome) {
        ...
        if (mortgagePayment <= 0 || mortgagePayment < annualInterestPaid) {
          ...
          annualMortgagePayment =
            (initialMortgagePrincipal * r * Math.pow(1 + r, n)) /
            (Math.pow(1 + r, n) - 1);
        }
        ...
        console.log(
          `Mortgage payment for year ${i}: $${annualMortgagePayment.toFixed(2)} (interest: $${annualInterestPaid.toFixed(2)}, principal: $${principalReduction.toFixed(2)})`
        );
        console.log(
          `Mortgage balance: $${mortgagePrincipal.toFixed(2)} → $${(mortgagePrincipal - principalReduction).toFixed(2)}`
        );
      ...
      if (hasCar) {
        ...
        if (carPayment <= 0 || carPayment < annualInterestPaid) {
          ...
          const n = carLoanTerm;
          annualCarPayment =
            (initialCarLoanPrincipal * r * Math.pow(1 + r, n)) /
            (Math.pow(1 + r, n) - 1);
        }
        ...
        console.log(
          `Car loan payment for year ${i}: $${annualCarPayment.toFixed(2)} (interest: $${annualInterestPaid.toFixed(2)}, principal: $${principalReduction.toFixed(2)})`
        );
        console.log(
          `Car loan balance: $${carLoanPrincipal.toFixed(2)} → $${(carLoanPrincipal - principalReduction).toFixed(2)}`
        );
      ...
      if (hasSpouse && spouseLiabilities > 0) {
        ...
        if (yearsIntoMarriage >= 0 && yearsIntoMarriage < spouseLoanTerm) {
          ...
          const initialSpouseLiabilities =
            marriageMilestone?.spouseLiabilities || 0;
          ...
          const amortizedAnnualPayment =
            (initialSpouseLiabilities * r * Math.pow(1 + r, n)) /
            (Math.pow(1 + r, n) - 1);
          ...
          console.log(
            `Spouse loan payment for year ${i}: $${amortizedAnnualPayment.toFixed(2)} (interest: $${annualInterestPaid.toFixed(2)}, principal: $${principalReduction.toFixed(2)})`
          );
          console.log(
            `Spouse loan balance: $${spouseLiabilities.toFixed(2)} → $${(spouseLiabilities - principalReduction).toFixed(2)}`
          );

          // Update spouse liabilities
          spouseLiabilities = Math.max(
            0,
            spouseLiabilities - principalReduction
          );
        }
      }
      ...
      if (hasSpouse && spouseAssets > 0) {
        ...
        if (yearsIntoMarriage > 0) {
          // Only apply growth after the first year of marriage
          ...
          console.log(
            `Spouse asset growth for year ${i}: $${assetGrowthAmount.toFixed(2)} (${spouseAssetGrowth}% of $${spouseAssets.toFixed(2)})`
          );
        }
      }
      ...
      const graduateSchoolDebt =
        projectionData?.graduateSchoolLoans?.[i - 1] || 0;
      const undergraduateSchoolDebt =
        projectionData?.educationLoans?.[i - 1] || 0;
      ...
      if (totalIncome < 50000) {
        ...
        taxRate = 0.2; // 20% for middle income
      } else {
    ...
    const currentExpenseCategories = {
      ...
      transportation:
        transportationExpensesData.length > 0
          ? transportationExpensesData[0]
          : 0,
      food: foodExpensesData.length > 0 ? foodExpensesData[0] : 0,
      healthcare:
        healthcareExpensesData.length > 0 ? healthcareExpensesData[0] : 0,
      education:
        educationExpensesData.length > 0 ? educationExpensesData[0] : 0,
      debt: debtExpensesData.length > 0 ? debtExpensesData[0] : 0,
      childcare:
        childcareExpensesData.length > 0 ? childcareExpensesData[0] : 0,
      discretionary:
        discretionaryExpensesData.length > 0 ? discretionaryExpensesData[0] : 0,
      // Make sure taxes are properly included with a significant non-zero value
      taxes:
        taxesData.length > 0 ? taxesData[0] : (incomeData[0] || income) * 0.2, // Use 20% default if needed
  ...
  const {
    ...
    isLoading: isLoadingSavedProjection,
    error: savedProjectionError,
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const cacheBuster = new Date().getTime();
      console.log(
        `Fetching projection data for ID: ${id} (cache bust: ${cacheBuster})`
      );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Failed to fetch projection data: ${response.status} ${response.statusText}`,
            errorText
          );
          ...
          throw new Error(
            `Failed to fetch saved projection: ${response.status} ${response.statusText}`
          );
        }
        ...
        console.log(
          "Successfully loaded projection data:",
          data.id,
          data.name,
          "with projectionData type:",
          typeof data.projectionData
        );
        ...
        if (data.projectionData && typeof data.projectionData === "string") {
          try {
            ...
            console.warn(
              "Could not parse projectionData string - will be handled during processing:",
              parseError
            );
          }
        }
  ...
  const effectiveCollegeCalc = useMemo(() => {
    ...
    if (fromJobPathway) {
      console.log(
        "Coming from job pathway - explicitly excluding all college calculations"
      );
      return null;
    }
    ...
    if (
      savedProjection &&
      savedProjection.collegeCalculationId &&
      collegeCalculations
    ) {
      const specificCollegeCalc = collegeCalculations.find(
        (calc) => calc.id === savedProjection.collegeCalculationId
      );
      if (specificCollegeCalc) {
        console.log(
          "Using college calculation from saved projection:",
          specificCollegeCalc
        );
  ...
  const effectiveCareerCalc = useMemo(() => {
    ...
    if (
      savedProjection &&
      savedProjection.careerCalculationId &&
      careerCalculations
    ) {
      const specificCareerCalc = careerCalculations.find(
        (calc) => calc.id === savedProjection.careerCalculationId
      );
      if (specificCareerCalc) {
        console.log(
          "Using career calculation from saved projection:",
          specificCareerCalc
        );
  ...
  const [projectionSummaryData, setProjectionSummaryData] =
    useState<ProjectionSummaryData | null>(null);
  const [isLoadingProjectionSummary, setIsLoadingProjectionSummary] =
    useState<boolean>(false);
  ...
  useEffect(() => {
      ...
      location: locationCostData
        ? {
            ...
            incomeAdjustmentFactor:
              locationCostData.income_adjustment_factor || undefined,
          }
        : undefined,
    ...
    console.log(
      "Updated projection summary data:",
  ...
  // Function to reset all projection-related state to defaults
  ...
  const loadProjection = async (projId: string) => {
    console.log("Loading projection with ID:", projId);
    try {
      ...
      const stateUpdates = () => {
        ...
        setExpenses(savedProj.expenses || 35000);
        ...
        // Parse the projection data with better error handling
        let parsedData;
        if (savedProj.projectionData) {
          try {
            // Handle both string and object formats
            if (typeof savedProj.projectionData === "string") {
              parsedData = JSON.parse(savedProj.projectionData);
            } else {
              parsedData = savedProj.projectionData;
            }
  ...
  useEffect(() => {
    ...
    if (projectionId) {
      console.log(
        "Loading projection with ID:",
        projectionId,
        "using loadProjection function"
      );
      loadProjection(String(projectionId));
    }
    ...
    if (didProjectionChange && savedProjection && !isLoadingSavedProjection) {
      ...
      const stateUpdates = () => {
        ...
        if (!hasCompleteData) {
          console.warn(
            "Projection has incomplete data - extracting from projectionData JSON"
          );
          ...
          try {
            ...
            if (typeof savedProjection.projectionData === "string") {
              try {
                ...
                projData = JSON.parse(
                  savedProjection.projectionData.replace(/\\"/g, '"')
                );
              }
            } else {
            ...
            const extractedAge =
              projData.ages && projData.ages.length > 0 ? projData.ages[0] : 25;
            const extractedSavings =
              projData.netWorth && projData.netWorth.length > 0
                ? projData.netWorth[0]
                : 5000;
            ...
            const extractedExpenses =
              projData.expenses && projData.expenses.length > 1
                ? projData.expenses[1]
                : 35000;

            // Set extracted values
            setTimeframe(
              projData.ages ? `${projData.ages.length - 1} Years` : "10 Years"
            );
            setAge(extractedAge);
            ...
            console.error(
              "Failed to extract data from projectionData JSON:",
              error
            );
          ...
          setTimeframe(
            savedProjection.timeframe
              ? `${savedProjection.timeframe} Years`
              : "10 Years"
          );
          ...
          setPersonalLoanInterestRate(
            savedProjection.personalLoanInterestRate || 8.0
          );
        }
        ...
        if (savedProjection.projectionData) {
          try {
            ...
            try {
              parsedData =
                typeof savedProjection.projectionData === "string"
              ...
              if (typeof parsedData === "string") {
                console.log(
                  "Detected double-stringified data, parsing again..."
                );
            ...
            if (!isValidProjectionData(parsedData)) {
              console.warn(
                "Invalid projection data structure detected. Creating default data instead of throwing error."
              );
              ...
              console.log(
                "Created default projection data with fields:",
                Object.keys(parsedData).join(", ")
              );
            } else {
              // Logging for successful parsing
              console.log(
                "Successfully parsed and validated projection data with fields:",
                Object.keys(parsedData).join(", ")
              );
            }
      ...
      if (effectiveCollegeCalc || effectiveCareerCalc || locationCostData) {
        console.log(
          "Updating projection summary data after loading projection..."
        );
        ...
        if (validateProjectionSummaryData(updatedSummaryData)) {
          ...
          console.error(
            "Invalid saved projection summary data:",
            updatedSummaryData
          );
          // Create a default summary data object using our utility function
          const defaultSummaryData = createDefaultProjectionSummaryData({
        ...
        console.log(
          "Projection summary data updated:",
          JSON.stringify(updatedSummaryData)
        );
      }
    }
  ...
  useEffect(() => {
    const updateProjectionData = async () => {
      console.log(
        "Inputs changed, calculating projection data using Python calculator"
      );
      try {
        // Add debugging for any existing milestones of type 'education'
        const educationMilestones =
          milestones?.filter((m) => m.type === "education") || [];
        console.log(
          "Education milestones for calculator:",
          educationMilestones
        );
        ...
        const retirementContributionRate = getAssumptionValue(
          "general",
          "retirement-contribution-rate",
          0.05
        );
        const retirementGrowthRate = getAssumptionValue(
          "general",
        ...
        const result = await calculateFinancialProjection(
          pythonInput,
          autoGenerate
        );
        console.log(
          "Received projection data from Python calculator:",
          result,
          "Auto-generate mode:",
          autoGenerate
        );
          ...
          milestones: formattedMilestones.map((m) => {
            ...
            const standardizedMilestone = {
              ...
              yearsAway:
                m.yearsAway !== null
                  ? m.yearsAway
                  : new Date(m.date as string).getFullYear() -
                    (new Date().getFullYear() - (age - 25)),
    ...
    if (!isInLoadingState) {
      ...
      console.log(
        "In loading state, skipping updateProjectionData - preserving original projection data"
      );
    }
  }, [
    income,
    ...
    autoGenerate,
    isInLoadingState,
  ]); // Include isInLoadingState to prevent recalculation when loading saved projections
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Change your location to recalculate financial projections based on
            the new cost of living. This change will be applied across all parts
            of the application.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          {searchByZip ? (
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-500">
                Enter a valid US 5-digit zip code. Try 90210 (Beverly Hills),
                02142 (Cambridge), 94103 (San Francisco), or 30328 (Atlanta) for
                example data.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Try popular cities like "San Francisco, CA", "New York, NY", or
                "Chicago, IL"
              </p>
            </div>
          )}
            ...
            disabled={
              ...
              (searchByZip
                ? !newZipCode ||
                  newZipCode.length !== 5 ||
                  !/^\d+$/.test(newZipCode)
                : !citySearchQuery || !selectedState)
            }
          >
            {updateZipCodeMutation.isPending || fetchingLocation
              ? "Updating..."
              : "Update Location"}
          </Button>
        </DialogFooter>

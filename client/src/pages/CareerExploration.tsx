...
import { FavoritesService, FavoriteCareer } from "@/services/favoritesService";
...
const CareerExploration = ({
  ...
  // Add local favorites state for optimistic updates
  const [localFavoriteCareers, setLocalFavoriteCareers] = useState<number[]>(
    []
  );
  ...
  // Query for user's favorite careers using FavoritesService
  const { data: favoriteCareers = [], isLoading: isFavoritesLoading } =
    useQuery<FavoriteCareer[]>({
  ...
  // Initialize local favorites from the fetched data
  useEffect(() => {
    if (favoriteCareers.length > 0) {
      setLocalFavoriteCareers(favoriteCareers.map((fav) => fav.careerId));
    }
  }, [favoriteCareers]);

  // Check if a career is favorited using local state for immediate feedback
  const isCareerFavorite = (careerId: number) => {
    return localFavoriteCareers.includes(careerId);
  };

  // Mutation to add a career to favorites using FavoritesService
  const addToFavoritesMutation = useMutation({
    mutationFn: async (careerId: number) => {
      if (!userId) {
        throw new Error("You must be logged in to favorite careers");
      }
      return FavoritesService.addCareerToFavorites(userId, careerId);
    },
    onSuccess: () => {
  ...
  // Mutation to remove a career from favorites using FavoritesService
  const removeFromFavoritesMutation = useMutation({
    mutationFn: async (favoriteId: number) => {
      if (!userId) {
        throw new Error("You must be logged in to remove favorite careers");
      }
      return FavoritesService.removeCareerFromFavorites(favoriteId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["/api/favorites/careers", userId],
  ...
  const filteredCareers = careers.filter((career) => {
    ...
    const matchesSearch =
      ...
      (career.alias1 &&
        career.alias1.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (career.alias2 &&
        career.alias2.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (career.alias3 &&
        career.alias3.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (career.alias4 &&
        career.alias4.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (career.alias5 &&
        career.alias5.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const toggleFavorite = (career: Career, event?: React.MouseEvent) => {
    // Prevent event propagation to avoid triggering parent element clicks
    if (event) {
    ...
    if (!isAuthenticated || !userId) {
      toast({
        ...
        description: "Please log in to add careers to your favorites.",
        variant: "destructive",
      });
      // Optionally redirect to login or show login dialog
      return;
    }

    // Find if this career is already favorited
    const favorite = favoriteCareers.find((fav) => fav.careerId === career.id);

    if (favorite) {
      // Optimistically remove from local state
      setLocalFavoriteCareers(
        localFavoriteCareers.filter((id) => id !== career.id)
      );
      ...
      removeFromFavoritesMutation.mutate(favorite.id, {
        onSuccess: () => {
          toast({
            title: "Success",
            description: `Removed ${career.title} from your favorites.`,
          });
        },
        onError: (error) => {
          // Rollback on error
          setLocalFavoriteCareers([...localFavoriteCareers]);
          console.error("Failed to remove favorite:", error);
          toast({
            title: "Error removing favorite",
            description:
              "Failed to remove career from favorites. Please try again.",
            variant: "destructive",
          });
        },
      });
      ...
      setLocalFavoriteCareers([...localFavoriteCareers, career.id]);
      ...
      addToFavoritesMutation.mutate(career.id, {
        onSuccess: () => {
          toast({
            title: "Success",
            description: `Added ${career.title} to your favorites.`,
          });
        },
        onError: (error) => {
          // Rollback on error
          ...
          toast({
            ...
            description: "Failed to add career to favorites. Please try again.",
            variant: "destructive",
          });
        },
      });
                          ...
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              if (checked) {
                                ...
                                setSelectedGrowth(
                                  selectedGrowth.filter((g) => g !== rate)
                                );
                              }
                              setCurrentPage(1); // Reset page when filter changes
                            }}
                        ...
                        <div className="flex flex-wrap items-center mt-2">
                          {career.growth_rate && (
                            <div className="flex items-center text-xs text-gray-500 mr-4 mb-1">
                              ...
                              {career.growth_rate === "fast"
                                ? "Growing fast"
                                : career.growth_rate === "stable"
                                  ? "Stable growth"
                                  : "Slow growth"}
                            </div>
                          )}
                        ...
                        <div className="mt-4 flex justify-between items-center">
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={(e) => toggleFavorite(career, e)}
                              disabled={

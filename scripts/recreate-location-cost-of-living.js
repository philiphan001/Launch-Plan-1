...
async function recreateLocationCostOfLiving() {
  ...
  try {
    ...
    for (let j = 0; j < Math.min(5, records.length); j++) {
      console.log(`Record ${j}:`, records[j]);
    }
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      let locationData = batch.map((record, index) => {
        ...
        // Use the real zip code from the CSV file
        // The key might be 'Zipcode' or have different casing or spacing
    ...
    // Update our San Francisco record with normalized data

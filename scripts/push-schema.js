...
try {
  ...
  // Create a sample SQL file to load data if needed
  const dataDir = path.join(process.cwd(), 'server', 'data');
  const sqlFilePath = path.join(dataDir, 'load-sample-data.sql');
...
`;
  ...
  const stream = createWriteStream(sqlFilePath);
  stream.write(sampleSql);
  stream.end();
  
  console.log(`Sample SQL file created at ${sqlFilePath}`);
  console.log('To load sample data, run: psql $DATABASE_URL -f server/data/load-sample-data.sql');
